...
flow-data: ## [DataOps] Full Data Cycle: Discovery -> Ingestion -> Meta -> Features -> Lakehouse
	@echo ">>> STAGE: DATA OPS (Discovery & Ingestion)"
	$(MAKE) scan-run
	$(MAKE) data-ingest
	$(MAKE) meta-ingest
	$(MAKE) feature-ingest
...
data-ingest: ## [DataOps] Ingest data for all candidates generated by scans
	@echo ">>> Ingesting data for candidate lists in export/$(RUN_ID)..."
	@# Find all candidate files from the scan run
	@find export/$(RUN_ID) -name "*.json" -print0 | xargs -0 -I {} $(PY) scripts/services/ingest_data.py --candidates "{}"
...
feature-ingest: ## [DataOps] Ingest TradingView Technicals for current candidates
	...
	@find export/$(RUN_ID) -name "*.json" -print0 | xargs -0 -I {} $(PY) scripts/services/ingest_features.py --candidates "{}"
