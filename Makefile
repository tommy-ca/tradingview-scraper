...
# --- DATA Namespace ---
flow-data: ## [DataOps] Full Data Cycle: Discovery -> Ingestion -> Repair -> Meta -> Features -> Lakehouse
	$(MAKE) scan-run
	$(MAKE) data-ingest
	$(MAKE) data-repair
	$(MAKE) meta-ingest
...
data-ingest: ## [DataOps] Ingest data for all candidates generated by scans
	...
	@find data/export/$(RUN_ID) -name "*.json" -print0 | xargs -0 -P 8 -I {} $(PY) scripts/services/ingest_data.py --candidates "{}"
	@echo ">>> Consolidating candidates to data/lakehouse/portfolio_candidates.json..."
	$(PY) scripts/services/consolidate_candidates.py --run-id $(RUN_ID)
...
meta-ingest: ## [DataOps] Refresh global metadata catalogs (Structural + Execution)
	...
	$(PY) scripts/fetch_execution_metadata.py --candidates data/lakehouse/portfolio_candidates.json
...
feature-ingest: ## [DataOps] Ingest TradingView Technicals for current candidates
	...
	@find data/export/$(RUN_ID) -name "*.json" -print0 | xargs -0 -P 8 -I {} $(PY) scripts/services/ingest_features.py --candidates "{}"
...
feature-backfill: ## [DataOps] Backfill historical technical ratings for dynamic backtesting
	...
	$(PY) scripts/services/backfill_features.py \
		--candidates data/lakehouse/portfolio_candidates.json \
