...
The goal is to maximize the net risk-adjusted return across all walk-forward windows.

### B. Implementation: `scripts/tune_parameters.py`
Utilize **Optuna** for Bayesian Optimization (TPE Sampler).
1.  **Sampler**: Tree-structured Parzen Estimator (TPE).
2.  **Pruner**: Median Pruner (stop trials that are significantly worse than the median).
3.  **Persistence**: Save study results to `data/optuna_study.db` for multi-run analysis.
