...
| Stage | Audit Evidence Captured (Structured Metrics) |
| :--- | :--- |
| **Discovery** | `n_discovery_files`: Count of non-empty JSON result files found in `export/`. |
| **Pruning** | `n_selected_symbols`: Number of leaders who survived the momentum gate. |
...
| **Recovery** | `trigger`: Evidence of failure that necessitated a recovery pass (e.g., `health_audit_fail`). |
| **Optimization** | `optimized_profiles`: List of profiles successfully generated by the solver. |
