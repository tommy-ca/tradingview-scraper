...
## Acceptance Criteria
- [ ] `backfill_features.py` output DataFrame has `UTC` timezone in index.
- [ ] `validate_data_contracts` passes for the backfilled data.
- [ ] Verify alignment with `returns` dataframe succeeds without dropping rows.
