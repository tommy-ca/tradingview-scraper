        ...
        for window_index, start_idx in enumerate(range(0, total_len - train_window - test_window + 1, step_size)):
            ...
            if use_ray and ray:
                if not ray.is_initialized():
                    ...
                    # Capture TV_ environment variables for workers
                    env_vars = {k: v for k, v in os.environ.items() if k.startswith("TV_")}
                    env_vars["PYTHONPATH"] = os.getenv("PYTHONPATH", "")
                    ray.init(ignore_reinit_error=True, local_mode=True)
                logger.info(f"Window {window_index}: Parallel optimization enabled (Ray PoC)")
            ...
            for eng in engines:
                ...
                for prof in results[sim_names[0]][eng].keys():
                    ...
                    try:
                        ...
                        if use_ray and remote_compute_weights and ray:
                            # Submit to Ray
                            futures[(eng, prof)] = remote_compute_weights.remote(
                                train_data, clusters, stats_df_final, prof, eng, cluster_cap, candidate_meta, cast(pd.Series, p_weights), regime, market_env, b_params
                            )
                        else:
                            if ledger:
            ...
            # Resolve Ray Futures
            if futures and ray:
                for (eng, prof), future in futures.items():
                    try:
                        res = ray.get(future)
                        if isinstance(res, pd.DataFrame) and not res.empty:
                            cached_weights[(eng, prof)] = res
                    except Exception as e:
                        logger.error(f"Ray Optimization failed for {eng}/{prof}: {e}")
