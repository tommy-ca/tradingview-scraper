            ...
            all_steps: List[Tuple[str, List[str], Optional[Callable[[], Any]]]] = [
                ...
                (
                    ...
                    [
                        "uv",
                        "run",
                        "scripts/services/ingest_data.py",
                        # We need to find the candidates generated by Discovery.
                        # They are in export/RUN_ID/*.json.
                        # The script accepts a single file or we can wrap it.
                        # Let's pass the directory or use a wrapper command?
                        # ingest_data.py takes --candidates.
                        # We can use a shell expansion if running via shell=True, but here we pass list.
                        # Best approach: Update ingest_data.py to accept a directory or glob,
                        # OR use a helper script, OR make 'make data-ingest' do the work.
                        # 'make data-ingest' uses 'find' and 'xargs'.
                        # Let's use make data-ingest.
                        "make",
                        "data-ingest",
                        f"TV_RUN_ID={self.run_id}",  # Pass RUN_ID so it looks in export/RUN_ID
                    ],
                    None,
                ),
