...
def build_barbell_portfolio():
    ...
    res_core = minimize(core_objective, init_weights, args=(core_returns,), method="SLSQP", bounds=bounds, constraints=constraints)
    ...
    for i, symbol in enumerate(core_candidates):
        portfolio.append({"Symbol": symbol, "Type": "CORE (Safe)", "Weight": res_core.x[i] * 0.90})

    # Add Aggressors (10% of total)
    for symbol in aggressors:
        portfolio.append({"Symbol": symbol, "Type": "AGGRESSOR (Antifragile)", "Weight": agg_weight})
