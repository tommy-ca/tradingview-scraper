...
def build_clustered_universe(*, returns: pd.DataFrame, clusters: Dict[str, Any], meta: Optional[Dict[str, Any]] = None, stats: Optional[pd.DataFrame] = None) -> ClusteredUniverse:
    ...
    # CR-215: Resolve structural mismatch in audit_clusters (dict vs list)
    # v3+ uses {"size": N, "selected": [syms]}
    ...
    if not avail: 
        logger.warning(f"build_clustered_universe: No available symbols in returns. winners_in_returns={len([s for s in returns.columns if s in all_syms])}")
        return ClusteredUniverse(pd.DataFrame(), {}, pd.DataFrame(), {}, {}, {})
    
    # 70% Coverage Gate
    thresh = len(returns) * 0.7; valid_cols = [s for s in avail if returns[s].count() >= thresh]
    if not valid_cols:
        counts = returns[avail].count()
        if not counts.empty:
            max_c = counts.max(); valid_cols = [s for s in avail if counts[s] >= max_c * 0.8]
    
    if not valid_cols: return ClusteredUniverse(pd.DataFrame(), {}, pd.DataFrame(), {}, {}, {})
