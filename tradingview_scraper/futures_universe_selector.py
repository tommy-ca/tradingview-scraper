...
class SelectorConfig(BaseModel):
    ...
    metadata: dict[str, Any] = Field(default_factory=dict)  # Metadata to inject into candidates
...
class FuturesUniverseSelector:
    ...
    def _apply_strategy_filters(self, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        # Phase 1350 Correction:
        # We MUST bypass legacy trend logic (_evaluate_trend) if intent is present,
        # BUT we MUST NOT bypass standard `filters` defined in the YAML.
        # Previously, we returned early if metadata['intent'] existed, skipping standard filters too.

        # 1. Evaluate Legacy Trend Logic (only if no intent override)
        # If intent is present, we consider the scanner "Dumb" regarding trend logic (it's handled downstream).
        skip_trend_logic = self.config.metadata and "intent" in self.config.metadata

        if skip_trend_logic:
            # Just pass through rows to the next step (which is _apply_filters in execute())
            # Wait, execute() calls _apply_strategy_filters AFTER _apply_filters?
            # Let's check call order in execute().
            # If execute() calls filters separately, then we are fine returning here.
            # But the YAML `filters` block is usually processed in _screen_market or _apply_basic_filters?
            # Let's check _build_filters.
            return rows

        # Legacy Support: Only apply if trend config has explicit direction/logic set
        has_legacy_filters = (
            self.config.trend_screen
            or self.config.confirm_screen
            or self.config.execute_screen
            or self.config.trend.adx.enabled
            or self.config.trend.recommendation.enabled
            or self.config.trend.momentum.enabled
        )

        if not has_legacy_filters:
            return rows
    ...
    def _load_market_cap_map(self) -> dict[str, float]:
        ...
        path_obj = Path(path)
        if not path_obj.exists():
            logger.warning("Market cap file not found: %s", path)
            self._market_cap_map = {}
            return self._market_cap_map
        caps: dict[str, float] = {}
        try:
                ...
                with path_obj.open("r", encoding="utf-8") as handle:
                    ...
                    for row in reader:
                        ...
                        if sym and cap_val is not None:
                            try:
                                score = str(sym).upper().replace(".P", "")
                                if ":" in score:
                                    score = score.split(":", 1)[-1]
                                for stable in sorted(STABLE_BASES, key=len, reverse=True):
                                    if score.endswith(stable) and len(score) > len(stable):
                                        score = score[: -len(stable)]
                                        break
                                caps[score] = float(cap_val)
                            except (TypeError, ValueError):
                                continue
        except Exception as exc:  # pragma: no cover - defensive
            logger.warning("Failed to load market cap file %s: %s", path, exc)
            caps = {}
