...
class Markets:
    ...
    def _build_payload(self, filters: Optional[List[Dict]] = None, columns: Optional[List[str]] = None, sort_by: str = "market_cap", sort_order: str = "desc", limit: int = 50) -> Dict:
        ...
        payload = {"columns": columns, "options": {"lang": "en"}, "range": [0, limit], "sort": {"sortBy": sort_field, "sortOrder": sort_order}}
    ...
    def get_top_stocks(self, market: str = "america", by: str = "market_cap", columns: Optional[List[str]] = None, limit: int = 50) -> Dict:
        """
        ...
        filters = [{"left": "type", "operation": "equal", "right": "stock"}, {"left": "market_cap_basic", "operation": "nempty"}]

        # Build payload
        payload = self._build_payload(filters=filters, columns=columns, sort_by=by, sort_order="desc", limit=limit)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not data:
                    return {"status": "failed", "error": f"No data found for market: {market}"}
                ...
                if self.export_result:
                    self._export(data=formatted_data, symbol=f"{market}_top_stocks", data_category="markets")
