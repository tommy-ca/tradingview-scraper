...
class AsyncStreamer:
    ...
    async def _add_indicators(self, indicators: List[Tuple[str, str]]):
        ...
        for idx, (indicator_id, indicator_version) in enumerate(indicators):
            ...
            # Note: fetch_indicator_metadata is currently synchronous
            # We might want to make it async later if needed
    ...
    async def get_data(self) -> AsyncGenerator[Dict[str, Any], None]:
        """
        Yields parsed data packets from the WebSocket handler.
        """
        while True:
